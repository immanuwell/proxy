...
static constexpr absl::string_view NetworkPolicyResourceTypeUrl =
    "type.googleapis.com/cilium.NetworkPolicyResource";

bool configSourceIsDelta(const envoy::config::core::v3::ConfigSource& config_source) {
...
public:
  ...
  bool insertOrUpdateSelectorResource(const std::string& key, const SelectorHandle& handle) {
    if (upserts_.contains(key)) {
      return false;
    }
    const auto* existing = findEntry(key);
    if (existing != nullptr && existing->selectorResourceEntry() == nullptr) {
      return false;
    }
    removed_.erase(key);
    return upserts_.emplace(key, ResourceKey::selectorResource(handle)).second;
  }
...
public:
  ...
  NetworkPolicyMapImpl(Server::Configuration::FactoryContext& context, bool use_nprds,
  ...
  void subscribe(std::unique_ptr<Envoy::Config::Subscription>&& subscription) {
    ...
    current_config_ = desired_config_;
    subscription_connected_ = false;
  }

  const envoy::config::core::v3::ConfigSource& getConfigSource() const {
    return current_config_.config_source_;
  }
...
protected:
  ...
  void setConfig(bool use_nprds, const envoy::config::core::v3::ConfigSource& config_source) {
    desired_config_ = XdsConfig{use_nprds, config_source};
  ...
  createOrReusePolicy(const std::string& resource_name, const cilium::NetworkPolicy& config,
                      ...
                      const ResourceMap& resource_map, bool is_new_stream,
                      const ResourceMapOverlay* pending_resource_map) const;

  std::pair<SelectorHandle, bool> createOrReuseSelector(const std::string& resource_name,
...
private:
  void onSubscriptionStreamEvent(uint64_t subscription_id, Config::GrpcMuxStreamEvent event) {
    ...
    case Config::GrpcMuxStreamEvent::Closed:
      ...
      // recreation until after it unwinds to avoid destroying the current subscription
      // mid-callback.
  ...
  void maybeRecreateSubscriptionInDesiredMode(bool transport_closed) {
    ...
    // Delta NPRDS or desired mode, or still connecting in desired mode.
    if (subscription_ && (subscription_connected_ || !transport_closed)) {
      if (subscription_connected_ && configSourceIsDelta(current_config_.config_source_) &&
          current_config_.use_nprds_) {
        // Keep Delta NPRDS on a connected subscription until transport closes.
        return;
      }
      if (current_config_.use_nprds_ == desired_config_.use_nprds_ &&
          Protobuf::util::MessageDifferencer::Equals(current_config_.config_source_,
                                                     desired_config_.config_source_)) {
  ...
  void onSubscriptionConnectedForTest() {
    onSubscriptionStreamEvent(subscription_id_, Config::GrpcMuxStreamEvent::Established);
  }
  void onSubscriptionTransportCloseForTest() {
    onSubscriptionStreamEvent(subscription_id_, Config::GrpcMuxStreamEvent::Closed);
  }
  ...
  // We need a separate desired_config_ as it may be set to a pessimistic value via explicit
  // BpfMetadata config in CiliumEnvoyConfig CRD, and we should not change to a "worse" (e.g., SotW)
  // ConfigSource if "better" (e.g., Delta NPRDS) is already up-and-running (in current_config_).
  XdsConfig desired_config_;
  XdsConfig current_config_;
...
// pass nullptr pending_resource_map for non-selector based policy!
std::pair<std::shared_ptr<const PolicyInstanceImpl>, bool>
NetworkPolicyMapImpl::createOrReusePolicy(
    ...
    bool is_new_stream, const ResourceMapOverlay* pending_resource_map) const {
...
std::pair<SelectorHandle, bool>
NetworkPolicyMapImpl::createOrReuseSelector(const std::string& resource_name,
  ...
  const auto* old_entry = resource_map.findEntry(resource_name);
  if (old_entry) {
    const auto* old_selector_entry = old_entry->selectorResourceEntry();
    if (old_selector_entry) {
      const auto& old_selector_handle = old_selector_entry->handle;
      if (old_selector_handle) {
        const auto* old_selector = old_selector_handle->get(update_version);
        if (old_selector &&
          ...
          return {old_selector_handle, true};
        }
      }
    }
  }
  ...
  return {selector_map_.insert(std::string(resource_name), selector), false};
}
...
NetworkPolicyMap::NetworkPolicyMap(Server::Configuration::FactoryContext& context, bool use_nprds,
                                   const envoy::config::core::v3::ConfigSource& config_source,
                                   bool subscribe)
    : context_(context.serverFactoryContext()) {
  impl_ = std::make_shared<NetworkPolicyMapImpl>(context, use_nprds, config_source);
...
void NetworkPolicyMap::setConfig(bool use_nprds,
                                 const envoy::config::core::v3::ConfigSource& config_source) {
  impl_->setConfig(use_nprds, config_source);
}
...
bool NetworkPolicyMap::subscriptionUseDeltaXdsForTest() const {
  return configSourceIsDelta(impl_->getConfigSource());
}
bool NetworkPolicyMap::desiredUseDeltaXdsForTest() const {
  return configSourceIsDelta(impl_->desired_config_.config_source_);
}
...
NetworkPolicyMapImpl::NetworkPolicyMapImpl(
    Server::Configuration::FactoryContext& context, bool use_nprds,
    const envoy::config::core::v3::ConfigSource& config_source)
    : context_(context.serverFactoryContext()), map_ptr_(nullptr),
      ...
      desired_config_(XdsConfig{use_nprds, config_source}), current_config_(desired_config_),
...
void NetworkPolicyMapImpl::subscribe() {
  ...
  if (subscription_factory_for_test_) {
    subscription_ =
        subscription_factory_for_test_(configSourceIsDelta(current_config_.config_source_));
  ...
  Config::OpaqueResourceDecoderSharedPtr decoder;
  if (current_config_.use_nprds_) {
    type_url = NetworkPolicyResourceTypeUrl;
    decoder = std::make_shared<NetworkPolicyResourceDecoder>();
  } else {
    type_url = NetworkPolicyTypeUrl;
    decoder = std::make_shared<NetworkPolicyDecoder>();
  }

  subscription_ = Cilium::subscribe(type_url, current_config_.config_source_, context_,
                                    *npds_stats_scope_, *this, decoder, std::move(on_stream_event));
...
void NetworkPolicyMapImpl::installNewPolicyMap(
  ...
  scheduleSelectorGCAndDeferredDeletion(new_version,
                                        exchange(new PolicyMapSnapshot(std::move(new_policy_map))));
}
...
absl::Status NetworkPolicyMapImpl::onConfigUpdate(
  ...
  try {
    ...
    if (is_new_stream) {
      ENVOY_LOG(info, "New NetworkPolicy{} stream {}", current_config_.use_nprds_ ? "Resource" : "",
                stream_generation);
      reopenIpcache();
    }

    // Handle NPRDS and NPDS separately
    if (current_config_.use_nprds_) {
      const auto selector_update_version = selector_map_.prepareNextVersion();
      absl::flat_hash_set<std::string> incoming_selector_names;

      // First validate and collect selector resource names
      for (const auto& resource : resources) {
        ...
        validateResourceName(resource_name, "NetworkPolicyResource resource name");

        switch (typed_resource.resource_case()) {
        case cilium::NetworkPolicyResource::kPolicy:
          validatePolicy(resource_name, typed_resource.policy(), version_info);
          break;
        case cilium::NetworkPolicyResource::kSelector:
          ENVOY_LOG(debug,
                    "Received NetworkPolicyResource selector {} in onConfigUpdate() version {}",
                    resource_name, version_info);
          incoming_selector_names.emplace(resource_name);
          break;
      ...
      // SotW resources are a complete snapshot. Selectors omitted from this snapshot are removed.
      for (const auto& [resource_name, resource_key] : resource_map_) {
        if (resource_key.selectorResourceEntry() &&
            !incoming_selector_names.contains(resource_name)) {
          selector_map_.clear(resource_name);
        }
      }

      // Add selectors before policies so policies can resolve selectors from the same snapshot.
      for (const auto& resource : resources) {
        const auto& typed_resource =
            dynamic_cast<const cilium::NetworkPolicyResource&>(resource.get().resource());
        ...
        const std::string& resource_name = resource.get().name();
        auto [selector_handle, reused] = createOrReuseSelector(
            resource_name, typed_resource.selector(), selector_update_version, resource_map);
        if (reused) {
          ENVOY_LOG(trace, "New selector is equal to old one, not updating.");
        }
        if (!pending_resource_map.insertOrUpdateSelectorResource(resource_name, selector_handle)) {
          throw EnvoyException(fmt::format(
              "NetworkPolicyResource update for version {} has duplicate resource key '{}': "
              "incoming selector resource '{}' collides with existing {}",
              version_info, resource_name, resource_name,
              pending_resource_map.describeExistingResourceKey(resource_name)));
        }
      }
      ...
      for (const auto& resource : resources) {
        const auto& typed_resource =
            dynamic_cast<const cilium::NetworkPolicyResource&>(resource.get().resource());
        if (typed_resource.resource_case() != cilium::NetworkPolicyResource::kPolicy) {
          continue;
        }
        const std::string& resource_name = resource.get().name();
        const auto& config = typed_resource.policy();
        auto [policy, reused] =
            createOrReusePolicy(resource_name, config, policy_stream_state, resource_map,
                                is_new_stream, &pending_resource_map);
        if (reused) {
      ...
      // NPDS just adds policy resources
      for (const auto& resource : resources) {
        ...
        auto [policy, reused] = createOrReusePolicy(resource_name, config, policy_stream_state,
                                                    resource_map, is_new_stream, nullptr);
        if (reused) {
          ENVOY_LOG(trace, "New policy is equal to old one, not updating.");
        }
        insertPolicyResource(
    ...
    ENVOY_LOG(warn, "NetworkPolicy{} update for version {} failed: {}",
              current_config_.use_nprds_ ? "Resource" : "", version_info, e.what());
    stats_.updates_rejected_.inc();
    removeInitManager();
    if (selector_map_.resetNextVersionIfNotChanged()) {
      scheduleSelectorDeferredDeletion(selector_map_.revert());
    }
    throw; // re-throw
  } catch (const std::bad_cast&) {
    ENVOY_LOG(warn, "NetworkPolicy{} update for version {} failed: invalid resource type",
              current_config_.use_nprds_ ? "Resource" : "", version_info);
    stats_.updates_rejected_.inc();
    removeInitManager();
    if (selector_map_.resetNextVersionIfNotChanged()) {
      scheduleSelectorDeferredDeletion(selector_map_.revert());
    }
    throw EnvoyException("Invalid resource type for Cilium NetworkPolicy");
  }
...
absl::Status NetworkPolicyMapImpl::onConfigUpdate(
  ...
  try {
    ...
    // Handle NPRDS and NPDS separately
    if (current_config_.use_nprds_) {
      // First validate and find whether this is a selector-only update.
      bool adds_policies = false;
      bool removes_policies = false;
      bool adds_selectors = false;
      bool removes_selectors = false;
      for (const auto& removed_resource : removed_resources) {
        ...
        auto resource_it = resource_map.find(removed_resource);
        if (resource_it == resource_map.end()) {
          continue;
        }
        if (resource_it->second.selectorResourceEntry()) {
          removes_selectors = true;
        } else if (resource_it->second.policyResourceEntry()) {
          removes_policies = true;
        } else {
          throw EnvoyException(fmt::format(
              "NetworkPolicyResource removed resource '{}' is a policy endpoint IP alias, "
              "not a resource name",
              removed_resource));
        }
      }
      for (const auto& resource : added_resources) {
        ...
        switch (typed_resource.resource_case()) {
        case cilium::NetworkPolicyResource::kPolicy:
          validatePolicy(resource_name, typed_resource.policy(), system_version_info);
          adds_policies = true;
          break;
        case cilium::NetworkPolicyResource::kSelector:
          ENVOY_LOG(trace,
      ...
      ENVOY_LOG(debug,
                "NetworkPolicyMapImpl::onConfigUpdate({}), "
                "version: {}, stream {}, updates_selectors: {}, updates_policies: {}",
                instance_id_, system_version_info, stream_generation,
                removes_selectors || adds_selectors, removes_policies || adds_policies);

      const auto selector_update_version = selector_map_.prepareNextVersion();

      // Is this a selector only update?
      if (!is_new_stream && (removes_selectors || adds_selectors) &&
          !(removes_policies || adds_policies)) {
        ...
        for (const auto& resource : added_resources) {
          ...
          case cilium::NetworkPolicyResource::kSelector: {
            ...
            if (!pending_resource_map.insertOrUpdateSelectorResource(resource_name,
                                                                     selector_handle)) {
              throw EnvoyException(fmt::format(
                  "NetworkPolicyResource selector update for version {} "
                  "has duplicate resource key '{}' on {} "
                  "stream: incoming selector resource '{}' collides with existing {}",
                  system_version_info, resource_name, is_new_stream ? "a new" : "an old",
                  resource_name, pending_resource_map.describeExistingResourceKey(resource_name)));
            }
            break;
          }
          case cilium::NetworkPolicyResource::kPolicy:
            IS_ENVOY_BUG(
                "Selector-only NetworkPolicyResource update unexpectedly included a policy");
            break;
          case cilium::NetworkPolicyResource::RESOURCE_NOT_SET:
            throw EnvoyException("NetworkPolicyResource has no payload");
          }
        }
      } else { // Policy and selector update path
        // First remove removed resources.
        for (const auto& removed_resource : removed_resources) {
          ...
          } else if (const auto* policy_entry = resource_entry->policyResourceEntry()) {
            pending_resource_map.erasePolicyResource(removed_resource, policy_entry->policy);
          }
        }
        // Then remove policies that are being replaced so duplicate endpoint IP detection sees the
        // pending state. Selector resources are updated in place below to preserve their stable
        // handles.
        if (adds_policies) {
          for (const auto& resource : added_resources) {
            const auto& typed_resource =
        ...
        // Then add selectors so policies can bind to selectors introduced by the same update.
        if (adds_selectors) {
          for (const auto& resource : added_resources) {
            const auto& typed_resource =
                dynamic_cast<const cilium::NetworkPolicyResource&>(resource.get().resource());
            ...
            if (!pending_resource_map.insertOrUpdateSelectorResource(resource_name,
                                                                     selector_handle)) {
              throw EnvoyException(fmt::format(
                  "NetworkPolicyResource selector update for version {} has duplicate resource "
                  "key '{}' on {} "
                  "stream: incoming selector resource '{}' collides with existing {}",
                  system_version_info, resource_name, is_new_stream ? "a new" : "an old",
                  resource_name, pending_resource_map.describeExistingResourceKey(resource_name)));
          ...
          for (const auto& resource : added_resources) {
            ...
            insertPolicyResource(pending_resource_map, config, system_version_info, is_new_stream,
                                 resource_name, policy, [](absl::string_view ip) {
                                   ENVOY_LOG(trace,
                                             "Cilium updating network policy for endpoint {}", ip);
                                 });
          }
        }
      }
    } else {
      // Delta NPDS carries only NetworkPolicy resources, so skip the selector scan/update paths.
      for (const auto& removed_resource : removed_resources) {
        validateResourceName(removed_resource, "NetworkPolicy removed resource name");
        ENVOY_LOG(trace, "Cilium removing NetworkPolicy {}", removed_resource);
        const auto* resource_entry = pending_resource_map.findEntry(removed_resource);
        if (resource_entry == nullptr) {
          continue;
        }
        if (const auto* policy_entry = resource_entry->policyResourceEntry()) {
          pending_resource_map.erasePolicyResource(removed_resource, policy_entry->policy);
          continue;
        }
        if (resource_entry->isPolicyEndpointIpEntry()) {
          throw EnvoyException(
              fmt::format("NetworkPolicy removed resource '{}' is a policy endpoint "
                          "IP alias, not a resource name",
                          removed_resource));
        }
        throw EnvoyException(fmt::format(
            "NetworkPolicy removed resource '{}' is not a policy resource", removed_resource));
      }

      // First validate and remove policies that are being replaced so duplicate endpoint IP
      // detection sees the pending state.
      for (const auto& resource : added_resources) {
        const std::string& resource_name = resource.get().name();
    ...
    ENVOY_LOG(warn, "NetworkPolicy{} update for version {} failed: {}",
              current_config_.use_nprds_ ? "Resource" : "", system_version_info, e.what());
    stats_.updates_rejected_.inc();
    removeInitManager();
    if (selector_map_.resetNextVersionIfNotChanged()) {
      scheduleSelectorDeferredDeletion(selector_map_.revert());
    ...
    ENVOY_LOG(warn, "NetworkPolicy{} update for version {} failed: invalid resource type",
              current_config_.use_nprds_ ? "Resource" : "", system_version_info);
    stats_.updates_rejected_.inc();
    removeInitManager();
    if (selector_map_.resetNextVersionIfNotChanged()) {
      scheduleSelectorDeferredDeletion(selector_map_.revert());
